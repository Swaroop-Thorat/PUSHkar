...
app.post('/push', async (req, res) => {
  try {
    ...
    if (!platform || !problemName || !code) {
      return res.status(400).json({ success: false, error: "need platform, problemName, and code" });
    }

    const sanitizedProblemName = problemName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
    const safeProblemName = problemName.replace(/\|/g, '-');
    ...
    // retry up to 3x on SHA conflict from concurrent pushes
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      const problemStr = `${problemNumber ? problemNumber + '. ' : ''}${safeProblemName}`;
